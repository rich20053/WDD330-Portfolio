...
Object.prototype.renderHikeDetail = function(item, isFull) 
{
    var strHTML = ` <div>
                    <h3>Distance</h3>
                    <p>${this.distance}</p>
...
{
  var makeHTML = ` <h2>${this.name}</h2>
  <div class="image"><img src="${imgBasePath}${this.imgSrc}" alt="${this.imgAlt}"></div>
  <div>";

  </div>`;
  item.innerHTML = makeHTML + renderHikeDetail(item);
  return item;
}
...
function renderOneHike(hike) {
    ...
    return hike.renderThisHike(item, false);
}